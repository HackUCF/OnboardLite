...
class Kennelish:
    ...
    def parse(self, obj, user_data=None):
        output = ""
        for entry in obj:
            try:
                if entry["input"] == "h1":
    ...
    def text(self, entry, user_data=None, inp_type="text"):
    ...
    def dropdown(self, entry, user_data=None):
        # Pre-filling of data from database
        if entry.get("prefill", True):
    ...
    def slider(self, entry, user_data=None):
...
class Transformer:
    ...
    def kennelish_to_form(self, json):
        obj = {}
