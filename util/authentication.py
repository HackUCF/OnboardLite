        ...
        async def wrapper(request: Request, token: Optional[str], *args, **kwargs):
            ...
            try:
                ...
                return Errors.generate(
                    request,
                    403,
                    "Invalid token provided. Please log in again and try again.",
                )
            ...
            # Is the current time after sudo permission expiration?
            # TODO(jeffrey) why is this a thing?
            if time.time() > creation_date + options.get("jwt").get("lifetime").get(
                ...
                return Errors.generate(
                    ...
                    essay="Unlike normal log-in, non-bot sudoer sessions only last a day. \
                        This is to ensure the security of Hack@UCF member PII. "
