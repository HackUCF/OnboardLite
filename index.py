...
async def oauth_transformer(redir: str = "/join/2"):
    ...
    oauth = OAuth2Session(
        ...
        redirect_uri=options.get("discord").get("redirect_base") + redir,
...
async def oauth_transformer_new(
    request: Request, response: Response, code: str = None, redir: str = "/join/2"
):
    # AWS dependencies
...
async def profile(request: Request, token: Optional[str] = Cookie(None)):
    if token is None:
        return RedirectResponse(
            "/discord/new/?redir=/profile", status_code=status.HTTP_302_FOUND
        )
    else:
